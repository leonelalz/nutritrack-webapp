...
/**
 * Detalle de un Plan Nutricional
 * US-16: Ver información detallada de un plan
 */
@Component({
  ...
  template: `
    ...
    @if (!loading() && plan()) {
      <div class="detalle-plan-container">
        <button class="btn-back" [routerLink]="['/catalogo/planes']">
          ← Volver a Planes
        </button>
...
export class CatalogoDetallePlanComponent implements OnInit {
  ...
  activarPlan(): void {
    if (!this.plan()) return;
    this.catalogoService.activarPlan({ planId: this.planId }).subscribe({
      next: (response: any) => {
        if (response.success) {
