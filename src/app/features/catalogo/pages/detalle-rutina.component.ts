...
/**
 * Detalle de una Rutina de Ejercicio
 * US-16: Ver información detallada de una rutina
 */
@Component({
  ...
  template: `
    ...
    @if (!loading() && rutina()) {
      <div class="detalle-rutina-container">
        <button class="btn-back" [routerLink]="['/catalogo/rutinas']">
          ← Volver a Rutinas
        </button>
...
export class CatalogoDetalleRutinaComponent implements OnInit {
  ...
  activarRutina(): void {
    ...
    this.catalogoService.activarRutina({ rutinaId: this.rutinaId }).subscribe({
      next: (response: any) => {
        if (response.success) {
